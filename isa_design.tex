...
An interesting feature of the ISA is the association of some instructions with two opcodes, named T1 and
...
Haskell specification, \textit{incAndFetchInstruction} is present, which is in charge of
updating the program counter register and fetching the next instruction.

Another instruction which might be of interest is the store, defined as STR (register).
...
the source specification are characterised with. There has not been loss of
...
A couple of examples will be showed along this section, in order to give to the reader
an idea of the \textit{graph extraction} outcome.
...
We did not implement the tool for the conversion yet.
