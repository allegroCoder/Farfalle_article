...
a visual graph-based representation composed by nodes and arcs, much more comrehensible 
...
\ref{sec:read}). As the basis for our experiment, we used microprocessor manual
\cite{armManual}, which includes short descriptions of each instruction as well as
...
An intersting feature of the ISA is that some instructions have two opcodes, named T1 and
...
second encoding in future work, in order to create a fully realistic benchmark (see Section
\ref{sec:frd}).
...
Going towards a more comprehensible ISA representation was one of our main goals. As discussed in Section
...
The following example clarifies the above issue. Listing
\ref{lis:and} gives a specification of the AND (register) instruction (ARMv6-M ISA
\cite{armManual}).\\
...
\ref{fig:andPO}. Every statement of the specification corresponds to a node in the
...
about operation type structure, decoding opcode/operands functions etc. For instance, in equation \ref{eq1},
we state that instruction is pair type with opcode and three operands, whereas i.e. opcode-decode function
can be defined as shown in equation \ref{eq2}.
...
opcode satisfies the guard of the event. In example, equation \ref{eq3} shows one of the guards of the ADD
instruction.
...
\bibitem{summevent}
	Jan Welte, Hansjörg Manz. \emph{``openETCS D2.1: Report on existing methodologies''}.
